...
- **Read, then run.** Every module pairs narrative with runnable code in `capstone/src/` and `capstone/tests/`. Apply ideas to the evolving FuncPipe RAG project.
...
- **Use the docs locally.** From the repository root, `make COURSE=python-programming/python-functional-programming docs-serve` renders this book and supports live reloading while you edit or experiment.
...
Track your progress by annotating the module markdowns or adding small “checkpoint” tests in `capstone/tests/` as you finish each core.

---

## Setup Checklist (Before Module 01)

1. Install the capstone tooling from the repository root: `make COURSE=python-programming/python-functional-programming install`.
2. Run the doc server from the repository root: `make COURSE=python-programming/python-functional-programming docs-serve`.
3. Run tests to ensure baseline correctness: `make COURSE=python-programming/python-functional-programming test`.
4. Skim `README.md` for the current project shape and module progression notes.
5. Inspect `capstone/README.md` if you want the runnable project entrypoints and directory map.
