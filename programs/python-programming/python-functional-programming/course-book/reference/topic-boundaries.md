...
graph LR
  reference["Reference"]
  center["Core functional discipline"]
  adjacent["Adjacent topics"]
  outside["Outside the course center"]
...
```mermaid
flowchart LR
  question["I have an FP design question"] --> center["Is it about purity, composition, data flow, or effect boundaries?"]
  center -->|yes| stay["Stay in this course"]
  center -->|partly| adjacent["Use this course plus an adjacent reference"]
  center -->|no| outside["Switch to a different course or source"]
```
<!-- page-maps:end -->
