...
flowchart TD
  ...
  module --> concept["Idempotent Effects"]
...
## 3. Public API – Idempotent Wrappers (`capstone/src/funcpipe_rag/domain/idempotent.py`)

```python
# capstone/src/funcpipe_rag/domain/idempotent.py – mypy --strict clean
...
```python
# capstone/src/funcpipe_rag/infra/adapters/atomic_storage.py
...
```python
# capstone/src/funcpipe_rag/fp/effects/io_retry.py – mypy --strict clean
from __future__ import annotations
...
- `capstone/src/funcpipe_rag/domain/idempotent.py` (`idempotent_write`)
- `capstone/src/funcpipe_rag/fp/effects/io_retry.py` (`retry_idempotent`)
...
Idempotence is verified with a deterministic fake capability in `capstone/tests/unit/domain/test_idempotent.py`.
...
**Continue with:** [Sessions and Transactions](../module-07-effect-boundaries-resource-safety/sessions-and-transactions.md)
