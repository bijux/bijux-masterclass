...
```mermaid
graph LR
  module["Persistence and Schema Evolution"]
  session["Session"]
  identity["Identity map"]
  aggregate["Aggregate"]

  module --> session --> identity --> aggregate
```

```mermaid
flowchart LR
  storage["Need persistence"] --> decide["Decide what the ORM owns and what the domain owns"]
  decide --> map["Map records to aggregates carefully"]
  map --> boundary["Keep session and lazy-loading boundaries visible"]
```
<!-- page-maps:end -->
