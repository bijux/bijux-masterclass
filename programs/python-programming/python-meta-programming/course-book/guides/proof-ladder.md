...
```mermaid
flowchart TD
  question["Name the exact claim you need to prove"] --> small["Start with the smallest honest command"]
  small --> inspect["Inspect the saved output or runtime surface"]
  inspect --> escalate["Escalate only if the smaller route is no longer enough"]
  escalate --> finish["Leave with evidence instead of intuition"]
```
<!-- page-maps:end -->
