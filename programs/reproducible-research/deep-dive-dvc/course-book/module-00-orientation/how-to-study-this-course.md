...
```mermaid
flowchart TD
  question["Start with the current state question"] --> lesson["Read the lesson that owns it"]
...
1. read the opening page until the state pressure is clear
...
Keep the Deep Dive DVC capstone as corroboration, not as first contact.

- use `capstone-walkthrough` when you want the bounded learner route
- use stronger verification, release, or recovery routes only when the current module is already legible
...
- state the main state rule or boundary in one sentence
