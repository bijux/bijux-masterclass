...
graph LR
  ...
  capstone["Capstone corroboration"]

  family --> program --> section --> page
  page -.checks against.-> capstone
```

```mermaid
flowchart LR
  title["Read the module title"] --> promise["State the promised learner outcome"]
  promise --> boundary["Name what is in and out of scope"]
  boundary --> proof["Choose the corroborating repository route"]
  proof --> review["Decide whether the title was actually delivered"]
```
<!-- page-maps:end -->
