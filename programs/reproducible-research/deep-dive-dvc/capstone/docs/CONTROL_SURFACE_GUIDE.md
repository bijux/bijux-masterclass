...
- run `dvc exp show` when the next question is how to compare a changed candidate against the baseline
- read [Release Review Guide](RELEASE_REVIEW_GUIDE.md) when the next question is whether a changed candidate deserves downstream trust
- read [Publish Contract](PUBLISH_CONTRACT.md) when the next question is which control surfaces are preserved in `publish/v1/`
