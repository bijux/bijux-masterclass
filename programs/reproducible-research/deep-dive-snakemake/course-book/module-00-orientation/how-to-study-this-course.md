...
```mermaid
flowchart TD
  question["Start with the current workflow question"] --> lesson["Read the lesson that owns it"]
...
1. read the opening page until the workflow pressure is clear
...
Keep the Deep Dive Snakemake capstone as corroboration, not as first contact.

- use `capstone-walkthrough` when you want the bounded learner route
- use stronger proof or audit routes only when the current module is already legible
...
- state the main workflow rule or boundary in one sentence
