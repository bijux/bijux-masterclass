...
1. `Makefile`
2. [Target Guide](target-guide.md)
3. [Proof Guide](proof-guide.md)
...
1. [Repro Guide](repro-guide.md)
2. `repro/*.mk`
3. [Incident Review Guide](incident-review-guide.md)
...
| --- | --- | --- |
| what is publicly supported | `Makefile` and [Target Guide](target-guide.md) | they define the durable learner-facing API |
...
| what the build must prove | `tests/run.sh` and [Selftest Guide](selftest-guide.md) | proof belongs to the harness, not to README prose |
| how failures are taught | `repro/` and [Repro Guide](repro-guide.md) | controlled defects should stay isolated from the main build |
...
* [Target Guide](target-guide.md) for the stable command surface
* [Proof Guide](proof-guide.md) for claim-to-evidence routing
* [Walkthrough Guide](walkthrough-guide.md) for the first-pass reading order
